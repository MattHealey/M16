...
library(lme4)
mort <- mortV2
mort.normal  <-       glm(deaths ~  offset(log(1+pop)) + age + eth + gen + quin + year + dhb + pop, data = mort)
mort.pois    <-       glm(deaths ~  offset(log(1+pop)) + age + eth + gen + quin + year + dhb + pop, data = mort, family = "poisson")
#mort.nb      <-    glm.nb(deaths ~  offset(log(1+pop)) + age + dhb + year + gen + eth + quin, data = mort)
mort.nb     <-     glm.nb(deaths ~  offset(log(1+pop)) + age + eth + gen + quin + year + dhb + pop, data = mort)
mort.zip     <-  zeroinfl(deaths ~  offset(log(1+pop)) + age + eth + gen + quin + year + pop| dhb, data = mort, link = "logit", dist = "poisson", trace = TRUE)
mort.zinb    <-  zeroinfl(deaths ~  offset(log(1+pop)) + age + eth + gen + quin + year + pop| dhb, data = mort, link = "logit", dist = "negbin", trace = TRUE, EM = FALSE)
